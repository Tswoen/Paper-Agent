...
<!-- > 维护排障记录请参考：[报错解决手册](./docs/error_resolution_manual.md) -->
...
- ⚙️ **系统配置页面**：支持在 Web 界面中维护模型 Provider、默认模型、智能体模型和嵌入模型，并可一键测试模型连通性
...
2. **配置环境**
   ...
   - 可以先手动修改 `src/core/models.yaml`，也可以启动 Web 界面后在“系统配置”页面中填写
3. **运行系统**
...
4. **Web界面**
   ...
   - 进入左侧菜单的“系统配置”，可配置模型服务商、默认模型、各智能体模型和嵌入模型

## 配置说明

### Web系统配置（推荐）

启动后端和前端后，访问 `http://localhost:5173`，在左侧菜单进入 **系统配置**。

在该页面中可以完成：

- 添加或修改模型 Provider，例如 OpenAI、SiliconFlow、DashScope、Ark 或其他 OpenAI 兼容服务
- 填写 API Key 和 API URL
- 设置默认 LLM、默认 Embedding 模型
- 为检索、阅读、分析、写作、报告生成等智能体单独指定模型
- 为知识库和聚类分析配置嵌入模型及向量维度
- 点击“测试连通性”检查当前模型是否可用

点击“保存配置”后，系统会自动更新 `src/core/models.yaml` 和 `.env`，并重新加载后端运行时配置。API Key 请填写真实密钥，不要填写环境变量名；保存后后端会自动把密钥写入 `.env`。
