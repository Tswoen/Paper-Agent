...
def download_paper_fulltext(
    ...
    *,
    ...
    """下载论文全文并写入公共缓存目录，下载失败时返回原因而不是抛出异常。"""
    ...
    try:
        with httpx.Client(timeout=timeout, follow_redirects=True, max_redirects=5) as client:
            with client.stream("GET", source_url, headers={"Accept": "application/pdf, text/html;q=0.9"}) as response:
                ...
                content = _read_limited_content(response, maximum_bytes)
                if content is None:
                    return DownloadedPaper(status="download_failed", reason="文件超过允许大小", source_url=final_url)
                content_kind = _detect_content_kind(content, response.headers.get("content-type", ""))
    ...
    (paper_dir / "source.json").write_text(
        json.dumps({"source_url": final_url, "content_type": response.headers.get("content-type", "")}, ensure_ascii=False),
        encoding="utf-8",
    )
    return DownloadedPaper(
        status="downloaded",
...
def _paper_cache_name(paper: PaperDocument) -> str:
    ...
    source = (paper.doi or paper.id or paper.title).strip().lower()
...
def _find_cached_file(paper_dir: Path) -> Path | None:
    ...
    for name in ("source.pdf", "source.html"):
...
def _read_limited_content(response: httpx.Response, maximum_bytes: int) -> bytes | None:
    """分段读取网络内容，超过限制时立刻停止并返回空值。"""
