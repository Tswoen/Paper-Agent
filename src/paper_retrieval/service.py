...
class PaperSearchService:
    ...
    这一层只负责三件事：
    1. 管理 connector 注册表；
    2. 将结构化检索意图转换成各 connector 需要的请求；
    3. 聚合、去重和截断结果。
    ...
    def search(
        ...
        2. 传 `topic` + `keywords`，由 connector 自己拼装最终查询串。

        中文注释：`truncate=False` 主要给图节点使用，让上层能先拿到所有来源的候选，
        再做统一评分排序和最终截断；默认仍保持旧接口的截断行为。
        """
    ...
    def _search_many(
        ...
        中文注释：每个来源都拿完整 limit，后续再由聚合、去重和评分排序统一截断。
        这样可以避免早期按来源均分导致某个高质量来源召回不足。
        """
