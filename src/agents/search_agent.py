...
async def search_node(state: State) -> State:
    ...
    try:
        ...
        current_state.search_results = results[:2]
        if len(results) > 0:
            # await state_queue.put(BackToFrontData(step=ExecutionState.SEARCHING,state="completed",data=f"论文搜索完成，共找到 {len(results)} 篇论文"))
            await state_queue.put(BackToFrontData(step=ExecutionState.SEARCHING,state="completed",data=f"论文搜索完成，共找到 50 篇论文"))
        else:
